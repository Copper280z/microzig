...
// this is a good place to substitute in hardware implementations of these approximations
// such as the ST CORDIC peripheral
...
pub const _PI: f32 = std.math.pi;
pub const _2PI: f32 = 2.0 * _PI;
pub const _PI_2: f32 = _PI / 2.0;
pub const _SQRT3_2: f32 = @sqrt(3.0) / 2.0;
...
pub fn _sincos(angle: f32, s: *f32, c: *f32) void {
    s.* = _sin(angle);
    c.* = _cos(angle);
}

// based on https://math.stackexchange.com/a/1105038/81278
pub fn _atan2(y: f32, x: f32) f32 {
    const abs_y = @abs(y);
    const abs_x = @abs(x);
    const a = @min(abs_x, abs_y) / (@max(abs_x, abs_y) + std.math.floatMin(f32));
    const s = a * a;
    var r =
...
test "atan2" {
    const tol: f32 = 0.0003;
    const y: f32 = -2.0;
    const x: f32 = -0.0;
    const ang = _atan2(y, x);
    try std.testing.expectApproxEqAbs(std.math.atan2(y, x), ang, tol);
}
